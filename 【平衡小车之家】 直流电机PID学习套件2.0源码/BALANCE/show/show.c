...
**************************************************************************/
unsigned char i,AWWW,temp;          //计数变量
...
void oled_show(void)
{
	if(Flag_OLED==1)                              //写入PID参数到Flash,由app控制该指令
	{
		if(++Clear_Count==1)OLED_Clear();           //清屏
  	OLED_ShowString(10,20,"Data Is Saved");     //提示数据已保存              	
		OLED_Refresh_Gram();				                //===刷新;
		if(Clear_Count>30) Flag_OLED=0,OLED_Clear(),Clear_Count=0; //约1.5s之后 恢复正常显示
	}
	
	if(Menu_MODE==1&&Flag_OLED!=1)//位置模式
	{
